...
if($searchValue != ''){
   $searchQuery = " and (id like '%".$searchValue."%' or 
        turbidityvalue like '%".$searchValue."%'  or tdsvalue like '%".$searchValue."%' ) ";
}
...
## Fetch records
$phQuery = "select id,turbidityvalue,tdsvalue,reading_time from departments WHERE department_id=1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
while ($row = mysqli_fetch_assoc($phRecords)) {
   $data[] = array( 
      "id"=>$row['id'],
      "turbidityvalue"=>$row['turbidityvalue'],
